...
export const MeasurementManagingPanel: React.FC = (props: any) => {
  ...
  >(
    (state) =>
      state.product.productMeasurementsState.managingMeasurementPanelContent
  );
  ...
  if (targetProductCategory) {
    switch (contentType) {
      ...
      case ManagingMeasurementPanelComponent.EditMeasurement:
        panelTitleText = 'Edit';
        panelDescription = [
          targetProductMeasurement ? targetProductMeasurement.name : '',
        ];
        panelWidth = 400;
        ...
        content = (
          <MeasurementForm
            productCategory={targetProductCategory}
            measurement={targetProductMeasurement}
            formikReference={formikReference}
            submitAction={(args: any) => {
              let action = assignPendingActions(
